...
type Evaluator interface {
	...
	RelinearizeDegree(cnIn *Ciphertext, ctOut *Ciphertext)
...
func (eval *evaluator) Mul2DegreeNew(op0, op1 Operand) (ctOut *Ciphertext) {
	ctOut = NewCiphertext(eval.params, 2, utils.MinInt(op0.Level(), op1.Level()), 0)
	eval.mul2Degree(op0, op1, ctOut)
	return
}
...
// mult degree 1 * degree 2
func (eval *evaluator) mul2Degree(op0, op1 Operand, ctOut *Ciphertext) {
	...
	if op0.Degree()+op1.Degree() > 3 {
		panic("cannot MulRelin: the sum of the input elements' degree cannot be larger than 3")
	}
	...
	var w00, w01, cp0, cp1, cp2, cp3 *ring.Poly
	...
	cp1 = ctOut.Value[1]
	if ctOut.Degree() < 3 {
		ctOut.El().Resize(eval.params.Parameters, 3)
	}
	cp2 = ctOut.Value[2]
	cp3 = ctOut.Value[3]
	...
	ringQ.MFormLvl(level, tmp0.Value[0], w00)
	ringQ.MFormLvl(level, tmp0.Value[1], w01)

	//ringQ.MulCoeffsMontgomeryLvl(level, w00, tmp1.Value[0], cp0) // c'0 = w[0]*c[0]
	//ringQ.MulCoeffsMontgomeryLvl(level, w01, tmp1.Value[1], cp2) // c'2 = w[1]*c[1]
	//ringQ.MulCoeffsMontgomeryLvl(level, w00, tmp1.Value[1], cp1) // w[0] * c[1]
	//ringQ.MulCoeffsMontgomeryAndAddLvl(level, w01, tmp1.Value[0], cp1) // c'1 = w[0]*c[1] + w[1]*c[0]
	ringQ.MulCoeffsMontgomeryLvl(level, w00, tmp1.Value[0], cp0)       // c'0 = w[0]*c[0]
	ringQ.MulCoeffsMontgomeryLvl(level, w00, tmp1.Value[1], cp1)       // w[0] * c[1]
	ringQ.MulCoeffsMontgomeryAndAddLvl(level, w01, tmp1.Value[0], cp1) // c'1 = w[0]*c[1] + w[1]*c[0]
	ringQ.MulCoeffsMontgomeryLvl(level, w00, tmp1.Value[2], cp2)       // w[0] * c[2]
	ringQ.MulCoeffsMontgomeryAndAddLvl(level, w01, tmp1.Value[1], cp2) // c'2 = w[0]*c[2] + w[1]*c[1]
	ringQ.MulCoeffsMontgomeryLvl(level, w01, tmp1.Value[2], cp3)       // c'3 = w[1] * c[2]
}
...
// Relinearize applies the relinearization procedure on ct0 and returns the result in ctOut. The input Ciphertext must be of degree three.
func (eval *evaluator) RelinearizeDegree(ct0 *Ciphertext, ctOut *Ciphertext) {
	if ct0.Degree() != 3 {
		panic("cannot Relinearize: input Ciphertext is not of degree 3")
	}
	...
	eval.SwitchKeysInPlace(level, ct0.Value[3], eval.rlk.Keys[0], eval.BuffQP[1].Q, eval.BuffQP[2].Q)

	ringQ.AddLvl(level, ct0.Value[1], eval.BuffQP[1].Q, ctOut.Value[1])
	ringQ.AddLvl(level, ct0.Value[2], eval.BuffQP[2].Q, ctOut.Value[2])
